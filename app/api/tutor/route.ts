...
function isValidBody(body: unknown): body is TutorRequestBody {
  ...
  if (
    ...
    b.messages.length === 0 ||
    b.messages.length > MAX_MESSAGES
  ) {
    return false;
  }
...
export async function POST(req: NextRequest) {
  ...
  if (
    body.context &&
    "expected_solution" in (body.context as unknown as Record<string, unknown>)
  ) {
    delete (body.context as unknown as Record<string, unknown>)[
      "expected_solution"
    ];
  }
  ...
  try {
    ...
    if (err instanceof TutorError) {
      ...
      return NextResponse.json(
        {
          error:
            "The study buddy is temporarily unavailable. Please try again shortly.",
        },
        { status: 502 },
      );
    }
